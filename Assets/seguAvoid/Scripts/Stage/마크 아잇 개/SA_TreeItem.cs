using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class SA_TreeItem : MonoBehaviour
{
    public bool arrived = false;

    public Vector2 p1;
    public Vector2 p2;
    public Vector2 p3;
    public Vector2 p4;

    public Vector2 p5;
    public Vector2 p6;
    public Vector2 p7;
    public Vector2 p8;

    float time = 0;
    public float moveTime;
    private void OnEnable()
    {
        time = 0;
    }

    private void Update()
    {
        if (transform.parent.parent.GetChild(1).GetChild(3).gameObject.activeSelf == false)
        {
            time += Time.deltaTime;
            if (time / moveTime < 1)
            {
                transform.position = Bezier(p1, p2, p3, p4, time / moveTime);
            }
            else
            {
                transform.position = Bezier(p5, p6, p7, p8, time / moveTime - 1);
            }
        }
    }

    public Vector2 Bezier(Vector2 p_1, Vector2 p_2, Vector2 p_3, Vector2 p_4, float value)   // value : 0 ~ 1의 float 값
    {
        Vector2 a = Vector2.Lerp(p_1, p_2, value);
        Vector2 b = Vector2.Lerp(p_2, p_3, value);
        Vector2 c = Vector2.Lerp(p_3, p_4, value);

        Vector2 d = Vector2.Lerp(a, b, value);
        Vector2 e = Vector2.Lerp(b, c, value);

        Vector2 f = Vector2.Lerp(d, e, value);

        return f;
    }
}
